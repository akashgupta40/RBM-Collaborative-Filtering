...
class RBM :
    def __init__(self, numhids, items, users, transactions):
		...
		for i in range(len(transactions)) :
			if i%100000==0 : 
				print i, 'Records completed..'
			self.matrix[self.u2h[transactions['userId'][i]]].append([self.i2h[transactions['movieId'][i]] , transactions['rating'][i]])

		self.matrix1 = [[] for i in range(self.nummachines)]
		...
		with open('u2h.pickle', 'wb') as handle:
		    pickle.dump(self.u2h, handle, protocol=pickle.HIGHEST_PROTOCOL)
		with open('i2h.pickle', 'wb') as handle:
		    pickle.dump(self.i2h, handle, protocol=pickle.HIGHEST_PROTOCOL)
		with open('h2u.pickle', 'wb') as handle:
		    pickle.dump(self.h2u, handle, protocol=pickle.HIGHEST_PROTOCOL)
		with open('h2i.pickle', 'wb') as handle:
		    pickle.dump(self.h2i, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def evaluate_RBM(self, test_data) :
        ...
        for i in range(len(test_data)) :
        	...
        	item = self.i2h[test_data['movieId'][i]]
        	rating = test_data['rating'][i]
        	self.matrix1[user].append([item,rating])
        with open('matrix1.pickle', 'wb') as handle:
            pickle.dump(self.matrix1, handle, protocol=pickle.HIGHEST_PROTOCOL)
        for i in range(self.nummachines) : 
        	err += self.cal_single_user_error(i)
